...
//!
//! * `counts_le3980.csv` — the model for `file_version < 3990`
//!   (FFmpeg cross-label `counts_3970`), and
//! * `counts_ge3990.csv` — the model for `file_version >= 3990`
//!   (FFmpeg cross-label `counts_3980`).
//!
...
/// (FFmpeg cross-label `counts_3970`). Monotonic `0 .. 65536`, length
/// `MODEL_ELEMENTS + 1`. Loaded from `src/tables/counts_le3980.csv`.
pub static COUNTS_LE3980: [u32; MODEL_ELEMENTS + 1] =
    parse_u32_col(include_str!("tables/counts_le3980.csv"));

/// Cumulative symbol-frequency table for `file_version >= 3990`
/// (FFmpeg cross-label `counts_3980`). Monotonic `0 .. 65536`, length
/// `MODEL_ELEMENTS + 1`. Loaded from `src/tables/counts_ge3990.csv`.
